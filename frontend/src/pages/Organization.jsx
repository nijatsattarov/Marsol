...
export default function Organization() {
  ...
  // Per-company selected event (companyId → eventId) for the quick-invite flow
  const [rowEventPick, setRowEventPick] = useState({});
  ...
  useEffect(() => { fetchObligations(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, []);
  ...
  const handleQuickInvite = async (companyId, status) => {
    const eventId = rowEventPick[companyId];
    if (!eventId) {
      toast.error('Əvvəlcə fəaliyyət seçin');
      return;
    }
    try {
      await axios.post(`${API}/events/${eventId}/quick-invite`,
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8" data-testid="organization-page">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* --- ÖHDƏLİKLƏR SİYAHISI (full-width bottom panel) --- */}
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5 mt-6" data-testid="obligations-panel">
          <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
            <div>
              <h2 className="text-lg font-bold text-[#3D4F6F]">Öhdəliklər siyahısı</h2>
              <p className="text-xs text-slate-500 mt-0.5">
                Şirkəti tap, fəaliyyəti seç, statusu qeyd et — bir kliklə dəvət olunur.
              </p>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <div className="relative">
                <Search className="w-4 h-4 absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400" />
                <Input value={oblSearch} onChange={(e) => setOblSearch(e.target.value)} placeholder="Şirkət / sahibkar axtar..." className="h-9 pl-8 text-sm w-64" data-testid="obl-search" />
              </div>
              <Select value={oblPackageFilter} onValueChange={setOblPackageFilter}>
                <SelectTrigger className="h-9 text-sm w-40" data-testid="obl-package-filter"><SelectValue /></SelectTrigger>
                <SelectContent>
          ...
          {oblLoading ? (
            <div className="flex justify-center py-8"><Loader2 className="w-6 h-6 animate-spin text-[#3D4F6F]" /></div>
          ) : (
            <div className="border border-slate-100 rounded-lg overflow-hidden overflow-x-auto" data-testid="obligations-table">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {obligations.filter(o => {
                    ...
                    return (
                      <tr key={o.company_id} className="border-t border-slate-100 hover:bg-slate-50" data-testid={`obl-row-${o.company_id}`}>
                        <td className="p-3">
                          <div className="font-medium text-[#3D4F6F]">{o.company_name}</div>
                          {o.owner_name && <div className="text-xs text-slate-500 mt-0.5">{o.owner_name}</div>}
                        </td>
                        ...
                        <td className="p-3 min-w-[200px]">
                          <Select
                            value={rowEventPick[o.company_id] || ''}
                            onValueChange={(v) => setRowEventPick(prev => ({ ...prev, [o.company_id]: v }))}
                          >
                            <SelectTrigger className="h-8 text-xs" data-testid={`obl-event-select-${o.company_id}`}>
                              <SelectValue placeholder="Fəaliyyət seç..." />
                            </SelectTrigger>
                            <SelectContent>
                              {events.map(ev => (
                        ...
                        <td className="p-3">
                          <div className="flex items-center gap-1.5 justify-center">
                            <button
                              ...
                              className="w-8 h-8 rounded-full bg-emerald-50 hover:bg-emerald-100 text-emerald-600 flex items-center justify-center transition-colors disabled:opacity-40"
                              disabled={!rowEventPick[o.company_id]}
                              title="Qatılır"
                            ...
                            <button
                              ...
                              className="w-8 h-8 rounded-full bg-red-50 hover:bg-red-100 text-red-600 flex items-center justify-center transition-colors disabled:opacity-40"
                              disabled={!rowEventPick[o.company_id]}
                              title="Qatılmır"
                            ...
                            <button
                              ...
                              className="w-8 h-8 rounded-full bg-amber-50 hover:bg-amber-100 text-amber-600 flex items-center justify-center transition-colors disabled:opacity-40"
                              disabled={!rowEventPick[o.company_id]}
                  ...
                  {obligations.length === 0 && !oblLoading && (
                    <tr><td colSpan={6} className="text-center p-6 text-slate-400">Öhdəlik siyahısı boşdur</td></tr>
                  )}
                </tbody>
              </table>
            </div>
