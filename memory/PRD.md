...
- **2026-02-14 İclas PDF overlap fix (final)**: Root cause aşkarlandı — `doc.text()` ilə çəkilən bölmə başlıqları (Gündəlik #N, Ümumi tapşırıqlar, Müzakirə mövzuları, Qərarlar) səhifə sonunda tək qalıb, cədvəl növbəti səhifəyə keçirdi; həmçinin `rowPageBreak:'auto'` uzun tapşırıq sətirlərini iki səhifə arasında bölürdü. Düzəliş: (1) `drawTasksBlock` və `drawListBlock` yenidən yazıldı — başlıq artıq autoTable-ın head-ində colSpan=5 sətri kimi. (2) `ensureSpace(50/25)` funksiyası hər blokdan əvvəl qalan səhifə sahəsini yoxlayır və lazım gələrsə `doc.addPage()` çağırır. (3) `rowPageBreak:'avoid'` sətirləri atomik saxlayır. (4) `showHead:'firstPage'` başlığın davam səhifələrində təkrarlanmasının qarşısını alır. Testing agent iteration_68 (RCA) → iteration_69 (PASS) stress-test-də (30 iştirakçı, 4 gündəlik × 2 tapşırıq, 3 ümumi + 6 mövzu + 5 qərar) təsdiqlədi.
