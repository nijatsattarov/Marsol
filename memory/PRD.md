...
- [x] **Şirkətlər Excel idxalı** — `POST /api/companies/import-excel` (multipart Excel); 2 sütun: Şirkət adı + Paket; mövcud şirkətlər brand_name (case-insensitive) ilə tapılır → paket yenilənir; yenilər default-larla yaradılır (`imported_via=excel` flag-i ilə); cavab: `{created, updated, skipped, total, errors[]}`; Companies header-ində "Import" düyməsi + toast bildirişi + auto-refresh — ✅ 2026-04-30 (Playwright smoke: 3 yeni, A=Premium, B=Business, C=Standart)
