...
- [x] **LSIM SMS inteqrasiya** — Yeni `sms_service.py` (LSIM Quick SMS REST API klienti, MD5 hash auth, unicode autodetect, AZ phone normalization 0XX/+994 → 994XXX); 8 yeni endpoint: `GET /api/sms/balance`, `GET/PUT /api/sms/templates/{key}`, `POST /api/sms/send` (manual), `POST /api/sms/bulk` (recipient_type ∈ companies/members/contacts + ids[], yaxud explicit recipients[]), `GET /api/sms/logs` (category filter), `GET /api/sms/logs/stats`, `POST /api/sms/dispatch-daily` (sabahkı tədbirlərə xatırlatma + bu günkü doğum günü təbriki, idempotent, sms_logs-da related_object_id ilə təkrarın qarşısı alınır); Yeni koleksiyalar: `sms_logs` (kim/kimə/mətn/transid/error_code/status/category/sent_by/related_object_id) və `sms_templates`; Sender ID: MARSOL (.env-də LSIM_LOGIN, LSIM_PASSWORD); Frontend: Settings → "SMS" tab — `SmsPanel` komponenti (balans card + 5 stat card + 2 şablon editoru + Avtomatik dispatch düyməsi + Test göndərmə formu + filterli tarixçə cədvəli); Companies səhifəsində bulk-action bara "Toplu SMS" düyməsi əlavə edildi (`BulkSmsModal` komponenti — şirkətlərin owner/representative/company telefonlarından unikal nömrələri çıxarır, GSM/Unicode SMS sayı hesablayır, xəta detalları açılır); admin-only permission — ✅ Iter 44 (25/25 pytest PASS, balance test sonrası 9284 SMS)
